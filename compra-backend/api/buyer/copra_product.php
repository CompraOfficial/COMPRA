<?php
    ...
    try{
        ...
        $traderProduct = mysqli_query($GLOBALS['connection'], "SELECT *, product.status AS pStatus FROM product JOIN trader ON product.trader_id = trader.trader_id WHERE product_id = {$pId}");
        if(mysqli_num_rows($traderProduct) > 0){
            while($product = mysqli_fetch_array($traderProduct)){
                $product_info = array(
                    ...
                    'status'=>$product['pStatus'] // (IN STOCK, OUT OF STOCK)
                );
